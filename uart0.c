...
void initUart0()
{
    ...
    UART0_CTL_R = UART_CTL_TXE | UART_CTL_RXE | UART_CTL_UARTEN;     //Print out these registers //10 million times a second
...
{
    while (UART0_FR_R & UART_FR_TXFF);               // wait if uart0 tx fifo full, get 0 if not full, else get the character
